...
@torch.no_grad()
def detect_unlabel_img(model, img, device,
                       certain_thre=0.8, uncertain_thre=0.3,
                       rejudge_iou_thre=0.7):
    # detect all results
    boxes, labels, scores = infer(model, img, device)

    # conf 分段, 分开 cer/uncer boxes, idxs 1D tensor
    keep_idxs = keep_idxs_by(scores >= uncertain_thre)
    ...
    uncertain_idxs = torch.tensor([idx for idx in keep_idxs if idx not in certain_idxs],
                                  dtype=torch.int64)
    # 保证保留下的 都是可信的 不会出现 同一 object 的 top classes 不同现象
    # 插入这些 box 进入数据库时 note='auto'，而人标注的 note='' & status=done
    if uncertain_idxs.size()[0] > 0:
        ...
        certain_idxs = rejudge_certain_anns(boxes, labels,
                                            certain_idxs, uncertain_idxs, iou_thre=rejudge_iou_thre)

    return boxes[certain_idxs], labels[certain_idxs]  # cpu tensor


@torch.no_grad()
def detect_unlabel_anns(model, batch_unlabel_anns, device,
                        certain_thre, uncertain_thre,
                        rejudge_iou_thre=0.5, gt_iou_thre=0.8):
    """
